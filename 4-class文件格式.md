...
如果一个`class`文件不带`StackMapTable`属性（版本号小于等于49.0），那么必须使用类型推断做校验。

#### 4.10.2.1 类型推断校验流程

在链接过程中，校验器对`class`文件中的每个方法执行数据流分析，对方法中的`Code`属性中的`code`数组进行检查。校验器确保在程序中任意一点，不管代码路径是如何到达该点的，以下条件都为真：

- 操作数栈尺寸不变，且包含的值的类型不变。
- 局部变量没有被访问到，除非已确定它包含的值拥有了适当的类型。
- 调用方法时使用了适当的参数。
- 给字段赋的值都有着适当的类型。
- 所有操作码的参数都有适当的类型，不管是操作数栈中的还是局部变量数组中的。

由于效率问题，某些原则上应该是校验器做的测试被延迟到了代码在方法中真正被调用的时候进行。这样，校验器就避免了在非必须的情况下加载`class`文件。

&emsp;&emsp;<sub>比如一个方法调用了另一个方法返回了一个*A*类的实例，而且这个实例只被赋值给了一个同样类型的字段，校验器没必要去检查类`A`是否真的存在。但如果把它赋值给了一个类型*B*的字段，*A*和*B*的定义必须要被加载进来，要确保*A*是*B*的子类。</sub>

#### 4.10.2.2 字节码校验器

每个方法的代码都是单独校验的。首先是把组成代码的字节拆成一个指令序列，还要把`code`数组中每条指令开始的索引放到一个数组中。然后校验器再次走查代码并解析指令。在这个过程中会构建一个数据结构用来保存方法中每条JVM指令的信息。指令如果有操作数，都得挨个检查，确保其有效性。比如：

- 分叉必须得在方法的`code`数组的界限范围内。
- 所有控制流指令的目标都得是一条指令的开始。如果是*wide*指令，*wide*操作码被认为是指令的开头，提供被*wide*指令修改的操作的操作码并不被认为是指令的开头。不允许分叉到一条指令的中间。
- 指令所访问或修改的局部变量的索引不能大于或等于该方法声明分配的局部变量的数量。
- 所有对常量池的引用必须要引用类型合适的记录。（比如*getfield*必须要引用一个字段。）
- 代码不能在一条指令的中间截止。
- 不能代码都没了还楞往后执行呢。
- 每个异常处理器所保护的代码起止点，起点必须是一条指令的开始，终点则恰好超过代码的末端。起点必须在终点之前。异常处理器的代码必须以一个有效的指令开始，但是不能以一个被*wide*指令修改的操作码开始。

对于方法中的每条指令，校验器会记录指令执行前操作数栈和局部变量数组的内容。对于操作数栈，需要知道栈的高度以及栈中每个值的类型。对于局部变量，要么得知道局部变量内容的类型，要么得知道这个局部变量包含了一个无用的或未知的值（可能是未初始化的）。在确定操作数栈中的值类型的时候，字节码校验器不需要区分各种整数类型（比如`byte`，`short`，`char`）。

然后，就要初始化一个数据流分析器。对于方法中的第一条指令，代表参数的局部变量一开始包含的值的类型是由方法的类型描述符声明的；此时操作数栈是空的。所有其它的局部变量包含的都是一个非法值。对于其它指令，还没有被检查到，从操作数栈或局部变量中也得不到什么信息。

最后，数据流分析器开始运行了。对于每条指令，有一个“已修改”比特位用来表示该指令是否需要被调查一番。一开始，只会设置第一条指令的“已修改”比特位。数据流分析器会执行下面的循环：

1. 选择一个设置了“已修改”比特位的JVM指令。如果没找到，说明方法已经成功完成校验。否则，关掉被选中指令的“已修改”比特位。
2. 通过以下操作，对该指令在操作数栈和局部变量数组的影响进行建模：
    - 如果指令使用了操作数栈的值，那就确保操作数栈有足够数量的值，且栈顶的值拥有适当的类型。否则，校验失败。
    - 如果指令使用了一个局部变量，确保这个局部变量包含的值拥有适当的类型。否则，校验失败。
    - 如果指令压栈，确保操作数栈有足够的空间，放得下新来的值。将它所蕴涵的类型添加到建模后的操作数栈顶。
    - 如果指令修改了一个局部变量，记录一下，这个局部变量现在包含了新的类型。
3. 判断哪些指令可以跟在当前指令后面。后继指令必须是以下其中之一：
    - 如果当前指令不属于无条件控制转移指令（比如*goto*、*return*、*athrow*），下一条爱是啥是啥。如果发现可能会直接“掉到”方法的最后一条指令之后，那么校验失败。
    - 一个有条件或无条件的分叉或switch的目标。
    - 该指令的任意异常处理器。
4. 在当前指令执行结束时，将操作数栈和局部变量数组的状态合并到每一条后继指令中，过程如下：
    - 如果这是后继指令第一次被访问到的话，记录一下第2步得到的操作数栈和局部变量的值是该后继指令执行前的操作数栈和局部变量数组的状态。为后继指令设置“已修改”比特位。
    - 如果该后继指令之前被访问过，将第2步得到的操作数栈和局部变量值合并到已有的值中。如果修改了这些值，那就设置一下“已修改”比特位。<br/>如果控制转移到了一个异常处理器：
    - 记录一个对象，类型就是异常处理器指明的异常类型，，它就是后继指令执行前操作数栈的状态。操作数栈必须有足够的空间容纳这个值，跟指令压栈一样。
    - 记录第2步之前的局部变量值就是后继指令执行前局部变量数组的状态。第2步中计算的局部变量值与此无关。
5. 继续第1步。

要想合并两个操作数栈，它们中的值的数量必须一致。然后比较两个栈中对应的值，计算出合并后的栈中对应的值，过程如下：

- 如果其中一个值是基本类型，它对应的值的类型必须是同样的基本类型。合并后的值也得是基本类型。
- 如果其中一个值是非数组的引用类型，它对应的值必须得是引用类型（数组或非数组）。合并后的值是一个实例的引用，它的类型是这两个引用类型的第一个公共父类型。（这种引用类型总是存在，因为`Object`类型是所有类、接口以及数组类型的父类。）

&emsp;&emsp;<sub>比如`Object`和`String`就可以合并；得到的结果是`Object`。同样，`Object`和`String[]`也可以合并；得到的结果也是`Object`。即便是`Object`和`int[]`也还是能合并，或者是`String`和`int[]`；得到的结果依然都是`Object`。</sub>

- 如果对应的值都是数组引用类型，那么就要检查它们的维度了。如果维度相同，那么合并后的值就是一个实例的`reference`，该实例是一个数组类型，它是这两个数组类型的第一个公共父类型。（如果两个数组类型中至少有一个的元素类型是基本类型，那么得到的元素类型就是`Object`。）如果数组维度不同，合并后的值是一个实例的`reference`，这个实例是一个数组类型，它的维度是二者中维度较小的那个数组的维度；其中的元素类型如果较小的数组是`Cloneable`或`java.io.Serializable`那么就跟它一样，否则就是`Object`。

&emsp;&emsp;<sub>比如`Object`跟`String[]`可以合并；结果是`Object[]`。`Cloneable`和`String[]`可以合并，或者是`java.io.Serializable[]`跟`String[]`也可以合并；结果分别是`Cloneable[]`和`java.io.Serializable[]`。即便是`int[]`和`String[]`也可以合并；结果是`Object[]`，因为在计算第一个公共父类的时候用`Object`替换了`int`。</sub>

&emsp;&emsp;<sub>因为数组类型的维度可能不同，`Object[]`和`String[][]`可以合并，或者是`Object[][]`跟`String[]`；得到的结果都是`Object[]`。`Cloneable[]`和`String[][]`可以合并；结果是`Cloneable[]`。最后，`Cloneable[][]`和`String[]`也可以合并；得到的结果是`Object[]`。</sub>

如果操作数栈无法合并，方法校验失败。

要合并两个局部变量数组状态，要比较对应的每一对儿局部变量。局部变量合并的计算规则也采用上面说的规则，只不过对应的值可以是不同的基本类型。此时，校验器会记录一下合并后的局部变量包含了一个不可用的值。

如果对方法执行数据流分析器的过程中没有报告校验失败，就说明该方法已经成功通过`class`文件校验器的校验了。

某些指令和数据类型会让数据流分析器变得有些复杂。下面我们就来具体的说一下。

#### 4.10.2.3 long与double类型的值

校验过程中要对`long`和`double`类型的值做特殊处理。

当一个`long`或`double`类型的值被移入位于索引*n*的局部变量时，要对索引*n*+1做特殊标记，标明它要为索引*n*处的值保留，而且不能用于局部变量索引。之前位于*n*+1处的不管是谁，都变成不可用的了。

当一个值被移入索引*n*处的局部变量时，要检查索引*n*-1，看看是不是有一个`long`或`double`类型的值。如果是，就要修改索引*n*-1的局部变量，标明它现在包含了一个不可用的值。因为索引*n*处的局部变量被覆盖了，那么索引*n*-1处就不能用来表达一个`long`或`double`类型的值了。

操作数栈上的`long`或`double`类型处理起来就简单一些；校验器把它们看成是栈上的单值。比如对于*dadd*操作码（对两个`double`值做加法）的校验代码会检查栈顶的头两个元素是否都是`double`类型。计算操作数栈的长度的时候，`long`和`double`类型的值占两个长度。

无类型指令在操作操作数栈的时候必须把`long`和`double`类型的值当作原子（不可分）。比如，如果栈顶元素是一个`double`而此时碰到了一条*pop*或*dup*指令，那么校验器就会报错。必须要使用*pop2*或*dup2*指令才行。

#### 4.10.2.4 实例初始化方法与新创建的对象

创建一个新的类实例包含多个步骤。下面的语句：
...
指令序列最后把刚创建好并完成初始化的对象留在了栈顶。（例子中其它部分的指令见§3（*编译*）。）

`myClass`类的实例初始化方法（§2.9.1）将局部变量0作为它的`this`参数，也就是这个新的未初始化的对象。在该方法调用`myClass`的其它实例初始化方法之前，或用`this`调用直接父类的实例初始化方法之前，该方法对`this`能干的事儿只有为`myClass`中声明的字段进行赋值。

对实例方法进行数据流分析的时候，校验器会将局部变量0初始化为当前类的一个对象，或者对实例初始化方法来说的话，就是让局部变量0包含一个特殊类型，表示一个未初始化的对象。当对象的一个适当的实例初始化方法被调用时（从当前类或它的直接父类），在校验器的操作数栈和局部变量数组模型中所有的该特殊类型出现的地方，都要换成当前类的类型。校验器会拒绝代码访问还没有被初始化的对象，或者是要重复初始化对象的代码。此外，它保证方法的每一次正常返回都已经调用了一个实例初始化方法，要么是该方法所在的类，要么是在直接父类中。

同样，*new*指令也会导致校验器的操作数栈模型中创建并压入一个特殊类型。这个特殊类型表示创建该类型实例的指令，以及尚未初始化的类实例的类型。在这个未初始化的类实例中，如果声明的实例初始化方法被调用了，那么所有出现特殊类型的地方都要替换成该类型实例对应的类型。随着数据流分析的进行，这种变更可能会传递给后续的指令。

指令数量也要保存在这个特殊类型中，因为在操作数栈中一个类可能会同时存在多个尚未初始化的实例。比如用来实现这段代码的JVM指令序列：

```
new InputStream(new Foo(), new InputStream("foo"))
```

在操作数栈上可能会有两个`InputStream`的未初始化实例。当实例初始化方法在类实例上被调用时，那些操作数栈或局部变量数组中出现特殊类型的地方，只有跟当前类实例是同一对象的地方才会被替换。

#### 4.10.2.5 异常与finally

要实现`try-finally`结构，Java语言的编译器生成版本号小于等于50.0的`class`文件的时候可能会用到异常处工具和两个特殊指令：*jsr*（“跳到子过程”）和*ret*（“从子过程中返回”）。在Java语言的方法中，`finally`子句会被编译成子过程JVM代码，跟异常处理器的代码很像。当*jsr*指令调用的子过程被执行时，它会把返回地址以及当前*jsr*指令的下一条指令的地址压栈，作为一个`returnAddress`类型的值。子过程的代码把返回地址存入局部变量。在子过程结束时，会有一个*ret*指令从局部变量中提出返回地址，并将控制转移到返回地址处的指令。

将控制转移给`finally`子句（可以调用的`finally`子过程）有多种方式。如果`try`子句正常完成，在计算下一条指令前，会通过一个*jsr*指令调用`finally`子过程。如果在`try`子句中有一个`break`或`continue`，那么在控制被转移到`try`子句外之前，会先执行一个*jsr*到`finally`子句的代码。如果`try`子句执行了一个*return*，编译后的代码是这样的：

1. 将返回值（如果有）保存到局部变量。
2. 执行一个*jsr*到`finally`子句的代码。
3. 当`finally`子句返回时，返回保存在局部变量中的值。

编译器会建立一个特殊的异常处理器，它会捕获`try`子句中抛出的任意异常。如果`try`子句中抛出了一个异常，这个异常处理器要：

1. 将异常保存到局部变量中。
2. 执行一个*jsr*到`finally`子句。
3. 当`finally`子句返回时，重新抛出该异常。

&emsp;&emsp;<sub>关于如何实现`try-finally`结构的更详细的内容见§3.13。</sub>

`finally`子句的代码为校验器提了一个特别的问题。通常，如果一个特定指令可以通过多条路径到达，而且一个特定局部变量由于这些路径会包含不兼容的值，那么局部变量就变成无用状态了。但是一个`finally`子句可能会在不同的地方调用，导致若干种不同的情况：

- 从异常处理器调用的话，得有一个局部变量包含一个异常。
- 实现*return*时调用的话，会有某些局部变量保存了返回值。
- 执行到`try`子句底部时调用的话，在同样的局部变量中会存在未确定的值。

`finally`子句本身的代码可能会通过校验，但是在更新完所有后续的*ret*指令后，校验器可能会发现，异常处理器本来认为应该保存异常的局部变量，或者是返回代码认为本来应该保存一个返回值的局部变量，现在保存的是一个未确定的值。

校验包含`finally`子句的代码是很复杂的。基本思路是这样的：

- 每条指令都要跟踪可以到达该指令的*jsr*目标列表。对于大部分代码来说这个列表都是空的。对于`finally`子句代码中的指令，这个列表长度为一。对于多层嵌套的`finally`代码（相当少见！），长度可能会大于一。
- 对于每个*jsr*指令要到达的那条指令来说，要用一个比特向量来维护该*jsr*指令开始执行后的所有被访问或被修改的局部变量。
- 执行*ret*指令的时候，它是要从一个子过程中返回，该指令只能从一个可能的子过程中返回。两个不同的子过程不能将它们的执行“合并”到一个单独的*ret*指令中。
- 要对*ret*指令做数据流分析的话，需要走一个特殊的流程。因为校验器知道该指令肯定是从哪个子过程返回，它可以找到所有调用该子过程的*jsr*指令，然后将*ret*指令执行时的操作数栈和局部变量数组的状态合并到*jsr*后面指令的操作数栈和局部变量数组中。合并过程用到了一组特殊的局部变量的值：

- 根据比特向量（上面建好的）给出的所有被子过程访问或修改的局部变量，使用它们在*ret*时的类型。
- 对于其它的局部变量，使用它们在*jsr*指令之前的类型。

## 4.11 JVM的限制

在`class`文件格式中存在以下JVM隐式限制：

- 因为`ClassFile`结构体（§4.1）的`constant_pool_count`字段是16位的，所以每个类或每个接口的常量池上限为65535。这便是一个单独的类或接口的整体复杂的的内部上限。
- 一个类或接口中可以声明的字段的数量上限是65535，这是由`ClassFile`结构体（§4.1）的`fields_count`字段的大小决定的。

&emsp;&emsp;注意，`ClassFile`结构体的`fields_count`字段值不包括从父类或父接口中继承的字段。

- 一个类或接口中可以声明的方法的数量上限是65535，这是由`ClassFile`结构体（§4.1）的`methods_count`字段的大小决定的。

&emsp;&emsp;注意，`ClassFile`结构体的`methods_count`字段值不包括从父类或父接口中继承的方法。

- 一个类或接口的直接父接口的数量上限是65535，这是由`ClassFile`结构体（§4.1）的`interfaces_count`字段的大小决定的。
- 在方法执行时，一个帧中的局部变量数量上限是65535（§2.6），这是由该方法该方法代码所属的`Code`属性（§4.7.3）的`max_locals`值的大小决定的，同时，JVM指令集的局部变量索引是16位的，也共同决定了这个结果。

&emsp;&emsp;注意`long`和`double`的值都要为其保留两个局部变量，占用`max_locals`值的两个单位，所以使用这种类型的局部变量会进一步降低上限。

- 一个帧（§2.6）的操作数栈大小上限为65535，这是由`Code`属性（§4.7.3）的`max_stack`值的大小决定的。

&emsp;&emsp;注意`long`和`double`的值都要占用`max_stack`的两个单位，使用这些类型的值也会进一步降低上限。

- 方法参数的数量上限为255，这是由方法描述符决定的（§4.3.3），在实例方法或接口方法调用时，这里面还包含了`this`占用的一个单位。

&emsp;&emsp;注意，方法参数长度会影响方法描述符的定义，`long`或`double`类型的参数要占用两个单位的长度，所以使用这种类型的参数的时候会进一步降低上限。

- 字段和方法名，字段和方法描述符，以及其它常量字符串值（包括`ConstantValue`（§4.7.2）属性引用的），上限为65535个字符，这是由`CONSTANT_Utf8_info`结构体的16位无符号`length`属性值的大小决定的。

&emsp;&emsp;注意这个限制指的是编码中用到的字节数，而不是编码后的字符数。UTF-8编码中有的字符要用两个或三个字节。所以使用这些多字节字符的字符串会进一步受限。

- 数组的维度上限位255，这是由*multianewarray*指令的`dimensions`操作码，以及*multianewarray*、*anewarray*、*newarray*指令（§4.9.1，§4.9.2）共同决定的。